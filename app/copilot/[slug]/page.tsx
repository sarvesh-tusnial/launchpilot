...
export default function CopilotPage() {
  ...
  useEffect(() => {
    if (loggedIn && activeTrack) loadTrackData(activeTrack)
  ...
  const loadCopilotProfile = async () => {
    ...
    setCopilotProfile(data.copilot)

    // Build track list
    const trackCodes = [data.copilot.track_1_code, data.copilot.track_2_code, data.copilot.track_3_code].filter(Boolean)
    const supabase = createClient()
    const { data: compData } = await supabase.from('competencies').select('code, name').in('code', trackCodes)
    setTracks(trackCodes.map(code => ({
      code,
      name: compData?.find((c: any) => c.code === code)?.name || code,
    })))
    setActiveTrack(trackCodes[0])

    // Check if already logged in
    const { data: { user } } = await supabase.auth.getUser()
    if (user && user.email === data.copilot.email) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (email.trim().toLowerCase() !== copilotProfile.email) {
  ...
  const openingMessage = copilotProfile && currentConcept
    ? `Hey ${copilotProfile.founder_name}! Ready to work on **${track?.name}**? We're on "${currentConcept.title}" — let's pick up where we left off.`
    : `Hey ${copilotProfile?.founder_name || 'there'}! Welcome to your co-pilot. Let's get started.`

  const sessionContext = copilotProfile
    ? `Co-pilot session for ${copilotProfile.founder_name}, founder of ${copilotProfile.business_name} (${copilotProfile.business_category}, ${copilotProfile.business_stage} stage, ${copilotProfile.country}). Business: ${copilotProfile.business_description}. Currently on track: ${track?.name}, concept: "${currentConcept?.title || 'first concept'}".`
    : ''
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '48px' }}>
            {tracks.map((t, i) => (
              <div key={t.code} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '12px 16px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: '10px' }}>
                ...
                <div>
                  <div style={{ fontSize: '13px', fontWeight: '500', color: '#C8C4BC' }}>{t.name}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ background: '#07050F', borderLeft: '1px solid rgba(127,119,221,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px 40px' }}>
        <div style={{ width: '100%' }}>
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <button type="submit" style={{ width: '100%', padding: '14px', borderRadius: '9px', border: 'none', background: '#7F77DD', color: '#fff', fontSize: '14px', fontWeight: '600', cursor: 'pointer', fontFamily: 'inherit', marginTop: '6px' }}>
              Enter →
            </button>
          </form>
  ...
  return (
    <div style={{ background: '#050309', minHeight: '100vh', fontFamily: "'DM Sans', system-ui, sans-serif", color: '#E8E6E0', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', flex: 1, paddingTop: '56px' }}>
        {/* SIDEBAR */}
        <aside style={{ width: '240px', flexShrink: 0, borderRight: '1px solid rgba(255,255,255,0.04)', padding: '24px 16px', position: 'fixed', top: '56px', bottom: 0, left: 0, overflowY: 'auto', background: '#050309' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', marginBottom: '20px' }}>
            {tracks.map((t, i) => {
              ...
              return (
                <button key={t.code} onClick={() => setActiveTrack(t.code)} style={{ width: '100%', textAlign: 'left', padding: '10px 12px', borderRadius: '8px', background: isActive ? 'rgba(127,119,221,0.08)' : 'transparent', border: isActive ? '1px solid rgba(127,119,221,0.18)' : '1px solid transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <span style={{ fontSize: '12px', fontWeight: '500', color: isActive ? '#E8E6E0' : '#555' }}>{t.name}</span>
                </button>
              )
            })}
          </div>
          ...
          {!loadingTrack && (
            <>
              <div style={{ padding: '14px', background: 'rgba(127,119,221,0.06)', border: '1px solid rgba(127,119,221,0.14)', borderRadius: '10px', marginBottom: '14px' }}>
                ...
                <div className="mono" style={{ fontSize: '9px', color: '#2a2a2a' }}>{masteredCount}/{totalConcepts} done</div>
              </div>
        ...
        {/* MAIN */}
        <main style={{ marginLeft: '240px', flex: 1, height: 'calc(100vh - 56px)', overflow: 'hidden' }}>
          {loadingTrack ? (
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
              <div className="mono" style={{ fontSize: '11px', color: '#222' }}>Loading...</div>
            </div>
          ) : (
            <MayaChat
