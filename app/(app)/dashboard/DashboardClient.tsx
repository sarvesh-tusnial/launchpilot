...
export default function DashboardClient({
  ...
  return (
    <div style={{ background: '#07040F', minHeight: '100vh', fontFamily: 'DM Sans, sans-serif', color: '#E8E6E0', display: 'flex', flexDirection: 'column' }}>

      {/* TOP NAV */}
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 200, height: '60px', background: 'rgba(7,4,15,0.96)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          {active && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <span style={{ fontSize: '12px', color: '#555', fontFamily: 'DM Mono, monospace' }}>{activeName}</span>
            </div>
          )}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <button onClick={signOut} style={{ fontSize: '11px', padding: '5px 12px', borderRadius: '7px', border: '1px solid rgba(255,255,255,0.07)', background: 'transparent', color: '#444', cursor: 'pointer', fontFamily: 'DM Mono, monospace' }}>
            Sign out
          </button>
      ...
      <div style={{ display: 'flex', flex: 1, paddingTop: '60px' }}>
        ...
        {/* LEFT SIDEBAR */}
        <aside style={{ width: '240px', flexShrink: 0, borderRight: '1px solid rgba(255,255,255,0.05)', padding: '24px 16px', position: 'fixed', top: '60px', bottom: 0, left: 0, overflowY: 'auto', background: '#07040F' }}>
          ...
          <div style={{ padding: '12px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '10px', marginBottom: '20px' }}>
            <div style={{ fontSize: '13px', fontWeight: '600', color: '#F0EDE6', marginBottom: '2px' }}>{profile.full_name || firstName}</div>
            <div style={{ fontSize: '11px', color: '#444' }}>{profile.job_title || 'Founder'}</div>
            <div style={{ marginTop: '10px', display: 'flex', gap: '16px' }}>
              ...
              <div>
                <div style={{ fontSize: '18px', fontWeight: '700', color: '#555' }}>{totalSteps - masteredConcepts}</div>
          ...
          {active ? (
            <div style={{ background: 'rgba(108,71,255,0.06)', border: '1px solid rgba(108,71,255,0.15)', borderRadius: '10px', padding: '14px', marginBottom: '16px' }}>
              ...
              <div style={{ fontSize: '13px', fontWeight: '600', color: '#E8E6E0', lineHeight: '1.3', marginBottom: '10px' }}>{activeName}</div>
              ...
              <div style={{ fontSize: '10px', color: '#444', fontFamily: 'DM Mono, monospace' }}>{masteredConcepts}/{totalSteps} steps completed</div>
            </div>
          ) : (
            <div style={{ padding: '16px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '10px', marginBottom: '16px' }}>
              <div style={{ fontSize: '13px', color: '#444', textAlign: 'center' }}>No pathway assigned yet</div>
            </div>
          )}
          ...
          {active && activeConcepts.length > 0 && (
            <>
              <div style={{ fontSize: '9px', fontFamily: 'DM Mono, monospace', color: '#333', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '8px', padding: '0 4px' }}>
                Steps
              </div>
                ...
                {activeConcepts.map((c: any) => {
                  ...
                  return (
                    <div key={c.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', padding: '6px 8px', borderRadius: '6px', background: isCurrent ? 'rgba(108,71,255,0.08)' : 'transparent', border: isCurrent ? '1px solid rgba(108,71,255,0.15)' : '1px solid transparent' }}>
                      <span style={{ fontSize: '10px', color: isDone ? '#00C851' : isCurrent ? '#6C47FF' : '#333', flexShrink: 0, marginTop: '1px' }}>
                        {isDone ? '✓' : isCurrent ? '→' : '○'}
                      </span>
                      <span style={{ fontSize: '11px', color: isDone ? '#333' : isCurrent ? '#E8E6E0' : '#555', lineHeight: '1.4', textDecoration: isDone ? 'line-through' : 'none' }}>
                        {c.title}
                      </span>
                    </div>
                  )
